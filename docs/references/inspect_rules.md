...
| `token_naming`             | WARNING          | A DDL object's database token should carry the kind suffix matching its type — tables (and their indexes/triggers) in a `{{*_T}}` token, views in a `{{*_V}}` token. Flags only a clear mismatch (e.g. a view in a `_T` token); a token with no kind suffix is left alone, and macro/procedure/function kinds (site-configurable) are enforced via a custom lint policy instead. Findings name the expected and actual suffix. (#172) |
...
| `data_dependent_change`    | WARNING          | An `ALTER TABLE` / `CREATE UNIQUE INDEX` on an existing table whose success depends on the data already present: adding `NOT NULL` without a `DEFAULT`, adding a `UNIQUE` constraint/index, adding a `CHECK` constraint, or changing `PRIMARY INDEX` / partitioning (data movement). Structurally valid but can fail or rewrite the table at deploy time. A `CREATE TABLE` (new, empty) is never flagged. Each finding carries `requires_live_metadata: true` and a `recommended_precheck` (e.g. a duplicate- or NULL-count query) so the risk can be assessed against the target environment. WARNING by default — set to ERROR in `inspect.conf` where prechecks are mandatory. `ALTER` inside a procedure body is exempt. (#170) |
| `non_linear_package_history` | WARNING        | A **project-level** check over the built packages under `releases/` (not a per-file rule). Flags a package sequence that cannot be trusted: an integrity sidecar (`.sha256`) that no longer matches its archive, a package that `requires` a sibling missing from its release group, an orphaned `prereqs` half with no `main`, a release group whose archives disagree on build number / timestamp, a build number reused across groups with different contents, or an older build number appearing after a newer one. No-op when `releases/` is absent. WARNING by default — set to ERROR for release/promotion workflows in `inspect.conf`. (#168) |
| `transaction_control_in_payload` | WARNING    | A transaction-control statement (`BT;` / `ET;`, `BEGIN TRANSACTION` / `END TRANSACTION`, `COMMIT`, `ROLLBACK`) in a payload file. Transaction boundaries are owned by the SHIPS deployer, so payload files should not open, commit, or roll back transactions. Detection runs on comment-stripped content (commented DI-tool workaround tokens never fire) and skips procedure/function `BEGIN … END` bodies, so an exception-handler `ROLLBACK` is exempt while a standalone `BEGIN TRANSACTION` is still caught. Findings identify the phase, file, line, and statement. WARNING by default; `--strict` promotes to ERROR for platform workflows. (#173) |
| `teradata_expensive_change` | WARNING         | A physical change that is syntactically valid but O(table size) and lock-heavy on large tables: `ALTER TABLE … ADD … DEFAULT` (full table rewrite) or `CREATE INDEX` (non-unique secondary index build). Cost depends on the live row count, so findings carry `requires_live_metadata`, a `recommended_precheck`, `possible_lock_impact`, `possible_spool_or_perm_impact`, and `requires_dba_review`. Deliberately does **not** double-flag the data-correctness cases (`CREATE UNIQUE INDEX`, `ADD … NOT NULL`, PI/partition changes → `data_dependent_change`) or `DROP`/recreate (`destructive_change`). `ALTER`/`CREATE INDEX` inside a procedure body is exempt. WARNING by default. (#175) |
| `contract_change`          | WARNING          | A **project-level** check (not a per-file rule) that compares the current source against a captured contract **baseline** and flags backward-incompatible changes that can break downstream consumers: removed / renamed / reordered **view** columns, removed **procedure** parameters or changed parameter direction/datatype, dropped or retyped **table** columns, or an object that disappeared (drop/rename). Added columns/parameters are backward-compatible and not flagged. **No-op until a baseline is captured** with `ships inspect --update-contract-baseline` (snapshots view columns / procedure parameters / table columns under `.ships/contracts.baseline.json`). WARNING by default; set to ERROR when comparing against a governed baseline / previous release. (#171) |
