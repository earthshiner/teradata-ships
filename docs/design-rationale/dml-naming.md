...
* **The manifest is the source of truth, not the filename.** SHIPS' `context/ships.build.json` records `primary_target`, `target_count`, and (where applicable) `target_candidates` and `confidence` for each placed file. The deployer uses these for ordering and gating decisions. The filename is a useful summary; the manifest is the contract.
