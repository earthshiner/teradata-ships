...
| `--output-dir` | Writes `env\DEV.conf` and `legacy_migration.sed` under this directory. |

3. Preview migration from legacy markers to SHIPS `{{TOKEN}}` markers.

```powershell
python -m td_release_packager migrate-source `
  --sed C:\Projects\OMR\config\legacy_migration.sed `
  --source C:\Legacy\sql `
...
| Argument | Explanation |
|---|---|
| `--sed` | Migration script generated by `import-legacy`. |
...
```powershell
python -m td_release_packager migrate-source `
  --sed C:\Projects\OMR\config\generated_token_migration.sed `
  --source C:\Generated\omr `
  --dry-run
...
| Argument | Explanation |
|---|---|
| `--sed` | Any `s/LHS/RHS/g` substitution script, including one you hand-write for token renames. |
