...
_EXT_VIEW = ".viw"
_EXT_DB = ".db"
_EXT_GRANT = ".dcl"  # PR-4 (issue #365): single canonical DCL extension.
_EXT_COMMENT = ".cmt"
...
def generate_grant_ddl(
    on_object_token: str,
    grantee_tokens: List[str],
) -> str:
    """
    Generate a consolidated DCL script for a single ON-object
    (the protected database receiving privileges).

    Grouping by ON-object (handover §7, issue #365) — grants are an
    attribute of the protected object and redeploy in that object's
    wave. The previous convention grouped by grantee.

    One ``GRANT SELECT ... WITH GRANT OPTION`` per grantee, merged
    and ordered through the canonical ``grant_merger`` so the
    output is deterministic and consistent with every other DCL
    file SHIPS produces.

    Args:
        on_object_token:  The protected database — the ``ON`` target
            of each statement. Used as the merge bucket and as the
            filename root upstream.
        grantee_tokens:   Databases receiving SELECT access.

    Returns:
        Full DCL text, one statement per grantee, sorted.
    """
    statements = [
        PrivilegeGrant(
            action="GRANT",
            privileges=("SELECT",),
            on_object=on_object_token,
            grantee=grantee,
            with_grant_option=True,
        )
        for grantee in set(grantee_tokens)
    ]
    merged = merge_statements(statements)
    header = [
        "-- Object Placement Standard read-path grants.",
        "-- Generated by tools/generate_view_layer.py",
        "-- ON-object: " + on_object_token,
    ]
    body = [emit_statement(s) for s in merged]
    return "\n".join(header + body) + "\n"
...
def run(
    ...
    # Grouping is by ON-object (the protected database) — see issue
    # #365, handover §7. ``collect_cross_module_grants`` returns
    # ``Dict[grantee, Set[grantor]]`` (the historical grantee-keyed
    # form); invert it to ``Dict[on_object, Set[grantee]]`` for the
    # ON-object-grouped emit.
    cross_module = collect_cross_module_grants(rewritten)

    on_object_to_grantees: Dict[str, Set[str]] = {}
    # Same-token-group grants — every ``_V`` reads from its sibling ``_T``.
    for grantee, grantor in grantees_needing_same_module.items():
        on_object_to_grantees.setdefault(grantor, set()).add(grantee)
    # Cross-module grants — flatten the grantee-keyed map.
    for grantee, grantors in cross_module.items():
        for grantor in grantors:
            on_object_to_grantees.setdefault(grantor, set()).add(grantee)

    for on_object in sorted(on_object_to_grantees):
        grantees = on_object_to_grantees[on_object]
        if not grantees:
            continue
        ddl = generate_grant_ddl(on_object, list(grantees))
        # Filename is the ON-object — the protected database. Whole-
        # name token in an unqualified slot; same shape as the views
        # database file above. ``.dcl`` is now the single canonical
        # extension for all DCL files (handover §7, G6).
        grant_filename = derive_filename_from_text(on_object, _EXT_GRANT)
