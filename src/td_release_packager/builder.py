    ...
    *,
    ...
    Args:
        ...
        archive_path:   Optional explicit output path for the archive.
            ...
            Use when ``internal_root`` differs from the basename and the
            on-disk archive filename must keep the long, fully-qualified
            ``<release_group>_<role>`` form.
    ...
    if archive_format == "tar.gz":
        # shutil.make_archive uses ``base_dir`` as the top-level folder
        # inside the archive.  When the caller wants a different
        # ``internal_root``, briefly rename ``pkg_dir`` so the archive
        # walk picks up the short name; restore on failure so the
        # caller's pkg_dir reference remains valid for cleanup.
        renamed_back = None
