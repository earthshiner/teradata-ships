...
def _clean_payload_tree(payload_base: str) -> int:
    """
    Wipe a payload tree by ``rmtree`` and recreate it empty.

    Replaces the previous per-file diff-based sweep, which compared the
    set of files this harvest produced against what was already on disk
    and removed the difference. That approach could leave stale entries
    behind when filenames differed between runs (e.g. differently
    tokenised ``.dcl`` filenames where the same logical object renders
    as ``{{DB_PREFIX}}_FOO.dcl`` in one run and ``{{DB_PREFIX_FOO}}.dcl``
    in another). The leftovers then broke a subsequent Package build's
    token-coverage scan.

    Using ``shutil.rmtree`` + recreate guarantees a re-harvest cannot
    inherit any artefact of a prior run, regardless of how filenames
    differ between runs. A ``.gitkeep`` marker is dropped into the
    recreated root so empty directories stay tracked.

    NOTE: hand-curated control files that used to live under
    ``payload/database/`` (e.g. a manually edited ``_order.txt``) are
    removed by this clean. They should live under ``config/`` instead;
    harvest regenerates the canonical ``_order.txt`` on every run.
    For overlay/incremental harvests that need to preserve existing
    payload contents, pass ``--keep-existing`` so this function is
    never called.
    ...
    Returns:
        Number of files removed (counted before the rmtree).
    """
    # Single shared primitive with the ships_clean MCP tool — see
    # td_release_packager.cleaner for the design rationale.
    from td_release_packager.cleaner import _rmtree_and_recreate

    files_removed, _dirs_removed = _rmtree_and_recreate(payload_base, seed_gitkeep=True)
