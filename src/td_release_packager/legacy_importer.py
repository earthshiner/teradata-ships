...
SHIPS project from chaos:
    ...
    2. tokenise.conf — the SHIPS tokenisation config. Converts
       source-side markers to the SHIPS ``{{TOKEN}}`` convention so
       the package is environment-independent and can be re-bound at
       deploy time. Harvest/process auto-apply this; run
       ``migrate-source --dry-run`` to preview.
...
Usage::
    ...
    #   ./MyProject/config/tokenise.conf
...
def format_migration_sed(subs: List[Substitution]) -> str:
    ...
    lines = [
        "# ===================================================================",
        "# tokenise.conf — generated by import_legacy_substitutions.py",
        "#",
        "# SHIPS tokenisation config. Converts source-side markers to the",
        "# SHIPS {{TOKEN}} convention so the package is environment-",
        "# independent and can be re-bound at deploy time. Loaded",
        "# automatically by harvest/process from <project>/config/tokenise.conf.",
        "#",
        "# Two rule forms are accepted:",
        "#",
        "#   s/LHS/RHS/g                       — literal substitution",
        "#                                       (what this generator emits)",
        "#",
        "#   regex::PATTERN:=REPLACEMENT       — regex with $1..$9",
        "#                                       capture-group back-references",
        "#                                       (hand-authored)",
        "#",
        "# Preview the effect of edits without rewriting files:",
        "#",
        "#   python -m td_release_packager migrate-source \\",
        "#       --tokenise-config <project>/config/tokenise.conf \\",
...
def scan_format_env_config_file(env: str, scan: ScanResult) -> str:
    ...
    return render_scaffold(
        ...
        next_steps=[
            ...
            "",
            "3. Apply the tokenisation rules to your source tree:",
            "     python -m td_release_packager migrate-source \\",
...
def _run_script_mode(args) -> int:
    ...
    migration_path = output_dir / "tokenise.conf"
