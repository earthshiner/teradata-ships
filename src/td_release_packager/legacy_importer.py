...
def format_properties_file(env: str, subs: List[Substitution]) -> str:
    ...
    Output is a flat list of ``KEY=value`` pairs with a header banner
    and per-token diagnostics for duplicates. Sectioning, derivation
    via ``{{TOKEN}}`` cascade, and grammar headers are NOT applied —
    they are human-judgement work the user does after import, using
    ``config/properties/DEV.properties`` as a structural template.
    ...
    lines = [
        "# ===================================================================",
        f"# {env}.properties — generated by import_legacy_substitutions.py",
        "#",
        "# Imported from a legacy sed substitution script. This is a FLAT",
        "# dump of (token, value) pairs — no sectioning, no derivation, no",
        "# grammar headers. Treat it as a starting point.",
        "#",
        "# Recommended next steps:",
        "#",
        "#   1. Open config/properties/DEV.properties (the canonical",
        "#      template) and use its 7-section structure as a model.",
        "#",
        "#   2. Identify your composition roots (ENV_PREFIX, SHIPS_ENV,",
        "#      SHIPS_PROJECT, INSTANCE, SECURITY_TIER) by inspecting",
        "#      the values below for the common prefix/infix patterns.",
        "#",
        "#   3. Convert literal database names below to derived form,",
        "#      e.g. PARENT_NODE=PDE_DEV_00 → {{ENV_PREFIX}}_{{SHIPS_ENV}}_{{INSTANCE}}",
        "#",
        "#   4. Cross-check against --env at build time by ensuring",
    ...
    for sub in subs:
        ...
        lines.append(f"{sub.var_name}={sub.value}")
