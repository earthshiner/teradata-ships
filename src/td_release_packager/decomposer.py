...
def format_env_config_file(env: str, result: DecompositionResult) -> str:
    ...
    from the inference. Sections 3-7 (users, SQL constants, engine
    flags, length policy, diagnostics) are emitted as empty
    placeholders for the user to populate from their other sources.
    ...
    # Section 1 content
    section_1_lines: List[str] = [
        f"SHIPS_ENV={roots.ships_env}",
        f"ENV_PREFIX={roots.env_prefix}",
    ]
    if roots.instance:
        section_1_lines.append(f"INSTANCE={roots.instance}")
    section_1_lines.append(f"SECURITY_TIER={roots.security_tier}")
    ...
    # Skip parent_node entries — PARENT_NODE was emitted explicitly
    # above; including the decomposed copy is a redundant duplicate.
    ...
    return render_scaffold(
        env=env,
        generator_label="decompose_database_names.py",
        source_label="literal database names (token_map.conf or names file)",
        next_steps=[
            "1. Verify composition roots in section 1 against your",
            "   platform topology. Low-confidence inferences are flagged",
            "   in the companion decomposition_report.md.",
            "",
            "2. Review the derived names in section 2 — token names were",
            "   generated from layer + kind segments. Rename to your",
            "   domain language (e.g. MDL_T → BASE_T) where it improves",
            "   readability.",
            "",
            "3. Sections 3-7 are empty — populate from your existing",
            "   knowledge of the project (users, SQL constants, etc.).",
            "",
            "4. Review outliers in section 8 (if any) and either rename",
            "   in source or move into the appropriate categorised section.",
        ],
        sections_content={
            1: "\n".join(section_1_lines),
            2: "\n".join(section_2_lines).rstrip(),
        },
        final_section_title=final_title,
        final_section_purpose=final_purpose,
        final_section_content=final_content,
    )
...
def format_decomposition_report(
    ...
    if result.collisions:
        ...
        for token, literals in sorted(result.collisions.items()):
            lines.append(
                f"- `{token}`: {', '.join(f'`{literal}`' for literal in literals)}"
            )
        lines.append("")
