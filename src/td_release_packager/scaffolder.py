...
def _generate_build_counter(project_dir: str, skip_existing: bool = False):
    """
    Generate the .build_counter file initialised to 0, under ``.ships/``.

    The builder reads and increments this file on each build.
    The file contains a single integer. It lives under the
    machine-managed ``.ships/`` directory and is ignored by git.
...
# Machine-managed SHIPS state (build counter, decisions log, analyse output).
# Each developer / CI pipeline maintains its own — do not commit.
.ships/
