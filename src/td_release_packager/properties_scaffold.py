...
``.properties`` template used by SHIPS env files.

The bootstrap tools (``import_legacy``, ``decomposer``) emit
``.properties`` files that downstream stages consume. Both tools
...
class Section:
    """One section of the canonical .properties scaffold.
...
#: Mirrors the structure of ``config/properties/DEV.properties``.
...
def render_scaffold(
    *,
    ...
    """
    Render the canonical 7-section ``.properties`` scaffold.
    ...
    Returns:
        Full ``.properties`` text, terminated with a newline.
    """
    lines: List[str] = []

    # --- Top banner ---
    lines.append("# " + "=" * 67)
    lines.append(f"# {env}.properties — generated by {generator_label}")
    lines.append("#")
