...
Reads a _waves.txt file where '---' barriers (or blank lines)
separate waves. Objects within the same wave have no mutual
dependencies and can execute in parallel across multiple streams.

File format:
    # Comment lines start with '#'

    # Wave 1 — tables (no dependencies)
    tables/DB.Table1.tbl
    tables/DB.Table2.tbl

    ---
    # Wave 2 — views (depend on tables above)
    views/DB.View1.viw
    views/DB.View2.viw

    ---
    # Wave 3 — macros (depend on views)
    macros/DB.Macro1.mcr

Validation:
    - No object may appear in more than one wave.
    - Every listed object must exist as a file.
    - Empty waves (two consecutive separators) are ignored.
"""
