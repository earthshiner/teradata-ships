#!/usr/bin/env python3
"""
bootstrap_env_config.py — Thin CLI shim for the
already-tokenised-source bootstrapper.

The engine lives at ``td_release_packager.env_config_bootstrapper``
so the main CLI (``python -m td_release_packager bootstrap-env-config``)
and this standalone shim share a single implementation.

Use this when your DDL is already tokenised (uses ``{{TOKEN}}``
references) but you don't yet have a ``.conf`` file with
values. It scans the project for referenced tokens and writes a
7-section ``.conf`` scaffold with all of them parked in
...
When to use this vs the other two bootstrappers:
    ...
    Source already uses {{TOKEN}} references     bootstrap-env-config

Usage::

    python tools/bootstrap_env_config.py \\
        --source ./MyProject \\
        --env DEV \\
...
from td_release_packager.env_config_bootstrapper import main  # noqa: E402
